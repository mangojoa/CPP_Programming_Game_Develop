...
/* 이제는 스택 메모리를 통해서 발생할 수 있는 메모리 오염에 대해서 알아보자 */
int* TestPointer() { // 이 함수는 스택 메모리 안에서 진행된 로직이기에 스택프레임이 종료되고 나면 의미가 없는 값이 되어버리는 함수이다.
	int a = 1;
	return &a;
}

// 반면에 이 안에서는 int* pointer의 값이 들어간다.
void TestWrong(int* ptr) { // 이를 ptr로 받아서 진행한다.
	int a[100] = {}; // 이 스택 프레임 안에서는 a[100]이 생성되며,
	*ptr = 0x12345678; // 이 안에서 *ptr의 값을 변경한다. 

	// 하지만 이 과정에는 우리는 void TestWrong(int* ptr) 함수가 스텍 프래임 안에서 생성되고 있다는 것을 잊지 말아야한다.
	// 이는 스택 프레임 안에서 원레의 int* pointer의 값을 수정하고 있으며 이는 곧 반환 될때(함수가 끝날때) 크래시가 발생한다.

	// 이러한 일은 개발을 진행하면서 발생할 수 있으며, 포인터나 참조값을 이용해서 함수에서 밖으로 반환하는 일이 자주 발생해서는 안된다.
	// + (원래도 이런 일이 있어서도 안되는 것이다.)
}
...
int main() {
	...
	*/
	...
	// 다중 포인터를 배우기 이전에 함수에서 포인터를 이용해 값을 바꾼다고 해도 스택프레임 영역이라서 종료 이후에는 의미가 없다는 것을 배웠다.
	// 하지만 이번에 다룰 것은 함수 = 스택 메모리 안에서 메모리가 오염이 될 수 있는 과정을 알아보려 한다.

	int* pointer = TestPointer(); // 이를 진행해도 pointer에 값은 변함이 없다.

	// 하지만 TestWrong(pointer); 를 진행하게 된다면 이야기가 달라진다.
