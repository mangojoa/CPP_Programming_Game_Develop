...
int main() {
	...
	v.end()와 같은 설격을 지니고 있는 상태에서 내가 진행하려고 했던 삭제와는 다른 양상으로 진행되기에
	이 메모리는 삭제를 진행한 순간부터 사용하기 힘든 메모리로 변할 가능성이 매우 높을 수 있다. 
	
	그리고 심각한 문제가 발생한다는 것인데 바로 v.erase(); 를 실행하는 순간 
	pointer 와 달리 iterator 가 들고 있는 또 다른 정보(_MyProxy)들에 손상이 발생한다는 것이다.
	이는 매우 크리티컬한 부분이다. 이렇게 삭제가 진행된 부분은 더 이상 사용할 수 없다는 것이 중요하다. */

	/* 그렇다면 이러한 부분을 고치기 위해서는 어떻게 해야하는가? 
	
	모든 데이터를 스캔하면서 3이라는 데이터가 있으면 일괄 삭제하고 싶다는 예를 통해서 알아보자. */
	// for (vector<int>::iterator it = v.begin(); it != v.end(); ++it) { 
	for (vector<int>::iterator it = v.begin(); it != v.end();) {
		int data = *it;

		if (data == 3) it = v.erase(it);
		/* 사실상 일반적인 v.erase(); 실행할 때 문제가 되는 것은 포인터가 유효하지 않은 값으로 변하기 때문에
		크리틱한 문제가 발생한다고 보면 되는 개념이다.
		하지만 이 문제를 해결할 수 있는 방법은 삭제된 iterator 값에 바로 값을 넣어주면 해결이 된다.
		그럼 iterator 가 가지고 있는 MyProxy의 값이 유효하게 남아있는 것을 확인할 수 있다.
		즉, 단독적으로 v.erase();를 실행하면 안된다는 것이다.

		하지만 이렇게 쉽게 넘어가서는 안되는 문제가 하나 더 남아있다.
		v.erase()를 통해서 데이터를 삭제하면 그 뒤의 데이터를 하나씩 당겨오는(복사) 과정을 거치게 되는데
		삭제된 자리에는 다음 데이터가 들어오게 되면서 이후의 데이터를 하나 놓치는 상황이 발생하게 된다.

		그럼 모든 데이터를 스캔하면서 삭제를 한다는 개념이 성립되지 않기에 이를 해결하기 위해서는
		근본적으로 작성된 for() 의 수정이 필요하다.

		유효한 데이터를 차례대로 스캔하면서 삭제하고 싶다면 ++it의 과정이 for () 안에 존재하는 것이 아닌 로직안에 존재해야한다. */
		else ++it;
		// 이렇게 진행한다면 추후의 모든 데이터를 정상적으로 스캔할 수 있는 전재가 성립될 수 있다.
	}
