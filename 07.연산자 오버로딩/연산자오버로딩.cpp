...
public:
    ...
    대입 연산자의 경우, 바로 자기 자신을 꺼내야하는 경우가 있다. 
    이런 경우, void 형식으로 반환하게 만든 대입 연산자의 경우, 사용할 수 없다.
    ...
    현업에서는 대입연산자를 사용하는 경우, void 를 사용하는게 아니라 
    Position& 를 사용하는 것이다. 
    */
    Position& operator=(int arg) {
        _x = arg;
    ...
    /* 복사 대입 연산자에 대해 알아보자.
    - 복사 대입 연산자 = 대입 연산자 중, 자기 자신의 참조 타입을 인자로 받는 연산자를 말하는 것이다.
    
    
    인자값이 자신과 같은 타입이라는 것이 특징이다.*/
    // [복사 생성자] [복사 대입 연산자] 등 특별 대우를 받는 이유는,
    // 말 그대로 객체가 '복사'되길 원하는 특징 때문이다.
    // 이를 만들어 주지 않으면 컴파일러가 만들어준다.
    Position& operator=(const Position& arg) {
        _x = arg._x;
        _y = arg._y;
...
int main() {
    ...
    */
    ...
    Position pos6;
    pos6 = 1; // 기존의 대입 연산자는 void 타입으로 반환되었지만 
    // 이를 바로 반환을 받아야하는 경우가, 생길 수 있다.

    // 그럴 경우 다음 과 같은 코드를 작성하기 마련이다.
    pos3 = (pos6 = 4);
    // 하지만 이는 자기 자신을 반환하는 타입의 함수로 작성되어야 한다.
    // 여기서는 (Position& x)의 타입으로 반환되며 리턴은 *this를 사용하면 된다.
